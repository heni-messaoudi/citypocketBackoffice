...
class DoctrineEntityGenerator extends Generator
{
    ...
    public function generate(BundleInterface $bundle, $entity, $format, array $fields)
    {
        ...
        $config->setEntityNamespaces(array_merge(
            array($bundle->getName() => $bundle->getNamespace().'\\EntitysControlers'),
            $config->getEntityNamespaces()
        ));

        $entityClass = $this->registry->getAliasNamespace($bundle->getName()).'\\'.$entity;
        $entityPath = $bundle->getPath().'/EntitysControlers/'.str_replace('\\', '/', $entity).'.php';
        if (file_exists($entityPath)) {
            throw new \RuntimeException(sprintf('EntitysControlers "%s" already exists.', $entityClass));
        }

        $class = new ClassMetadataInfo($entityClass, $config->getNamingStrategy());
        $class->customRepositoryClassName = str_replace('\\EntitysControlers\\', '\\Repository\\', $entityClass).'Repository';
